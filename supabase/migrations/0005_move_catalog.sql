...
-- packages/engine/test/seed.test.ts fails if the two drift apart.
--
-- Reference data lives in a migration, not in seed.sql: `supabase db push`
-- applies migrations to a hosted project and never runs the seed, and a
-- deployment without a move catalog is a deployment where nobody can fight.
